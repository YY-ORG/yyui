import {Component, Input, Output, EventEmitter, OnChanges, SimpleChanges, ElementRef, ViewEncapsulation, Renderer,OnInit} from '@angular/core';
...
export class MyDatePicker implements OnChanges ,OnInit {
    @Input() options:any;
    @Input() locale:string;
    ...
    setInitDate(date?: string):void {
        ...
        const format = this.initDate.split("-").map(v => ""+(+v));
        this.selectDate({year: format[0], month: format[1], day: format[2]});
    }
